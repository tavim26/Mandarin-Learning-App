...
import { getLessonLeaderboard, type StudentLessonProgressDto } from '@/api/progressApi';
import {
  BarChart,
  Bar,
  XAxis,
...
const TeacherLessonPage = () => {
  ...
  const [lessonLeaderboard, setLessonLeaderboard] = useState<StudentLessonProgressDto[]>([]);
  ...
  try {
    setLoading(true);
    const [lessonData, exercisesData, materialsData, leaderboardData] = await Promise.all([
      getLesson(parsedLessonId),
      getExercisesByLesson(parsedLessonId),
      getMaterialsByLesson(parsedLessonId),
      getLessonLeaderboard(parsedLessonId),
    ]);
    setLesson(lessonData);
    setExercises(exercisesData);
    setMaterials(materialsData);
    setLessonLeaderboard(leaderboardData);
  } catch {
  ...
  const handleDeleteExercise = async () => {
    ...
    await deleteExercise(deleteExerciseTarget.id);
    ...
    <>
      {showCreateExercise && (
        <ExerciseModal
          ...
          onSave={handleCreateExercise as never}
        />
      )}
      {editExercise && (
        <ExerciseModal
          ...
          onSave={handleEditExercise as never}
        />
      )}
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <Link
            to={`/teacher/units/${lesson.unitId}`}
            className="font-medium transition-colors hover:opacity-70"
            style={{ color: '#e85d04' }}
          >
            Unit #{lesson.unitId}
...
<div className="space-y-4">
  ...
  >
    ...
    <span className="ml-2 text-sm font-normal text-gray-400">
      ({lessonLeaderboard.length} students)
    </span>
  </h2>
    ...
    {lessonLeaderboard.length === 0 ? (
      <div className="flex items-center justify-center h-40">
        <p className="text-sm text-gray-400">
          No students have started this lesson yet.
        </p>
      </div>
    ) : (
              ...
              {exercises.map((ex) => (
                ...
                >
                  <div className="flex items-start gap-4 min-w-0">
                    <span
                      className="text-xs font-bold px-2.5 py-1 rounded-md flex-shrink-0 mt-0.5"
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                    ...
                    <button
                      onClick={() => setDeleteExerciseTarget(ex)}
              ...
              {materials.map((mat) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="text-xs font-semibold px-3 py-1.5 rounded-lg transition-all hover:bg-red-50 flex-shrink-0 ml-4"
                    style={{ color: '#c1121f' }}
                  >
                    Delete
                  </button>
