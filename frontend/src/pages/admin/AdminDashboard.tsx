...
import { useAdminOverview } from '@/hooks/useUsers';
import { useAllStudentsProgress } from '@/hooks/useProgress';

const AdminDashboard = () => {
  const navigate = useNavigate();
  const { users, totalStudents, totalTeachers, totalAdmins, loading: usersLoading, error } = useAdminOverview();
  const { students: progress, loading: progressLoading } = useAllStudentsProgress();

  const loading = usersLoading || progressLoading;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 space-y-4" style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.07)' }}>
          ...
          {progress.length === 0 ? (
            <p className="text-sm text-gray-400 py-4 text-center">No student activity yet.</p>
          ) : (
              ...
              {progress.map((student, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-800">Student #{student.studentId}</p>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="text-left">
              ...
              <p className="text-sm text-gray-400 mt-0.5">View, create and delete users</p>
            </div>
            <span className="text-2xl font-thin transition-transform group-hover:translate-x-1" style={{ color: '#e85d04' }}>
              →
            </span>
          </button>
              ...
              {users.slice(0, 5).map((user) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      background: user.role === 'ADMIN' ? '#fff7f0' : user.role === 'TEACHER' ? '#f0f9ff' : '#f0fdf4',
                      color:      user.role === 'ADMIN' ? '#e85d04' : user.role === 'TEACHER' ? '#0369a1' : '#15803d',
                    }}
                  >
                    {user.role}
                  </span>
