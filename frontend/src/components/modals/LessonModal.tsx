...
interface LessonModalProps {
  unitId: number;
  initial: Partial<LessonDto> | null;
  onClose: () => void;
  onSave: (data: Omit<LessonDto, 'id' | 'exercises'>) => Promise<void>;
}

const LessonModal = ({ unitId, initial, onClose, onSave }: LessonModalProps) => {
  const [title, setTitle] = useState(initial?.title ?? '');
  const [description, setDescription] = useState(initial?.description ?? '');
  const [xpReward, setXpReward] = useState<string>(initial?.xpReward?.toString() ?? '100');
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">XP Reward</label>
              <Input type="number" min={0} className="h-11 rounded-xl border-gray-200 bg-gray-50" value={xpReward} onChange={(e) => setXpReward(e.target.value)} />
            </div>
            <div className="space-y-1.5">
