...
class TranslationService:
    ...
    def translate(self, text: str, target_language: str) -> str:
        ...
        try:
            ...
            raise TranslationException("Google Translate API nu a raspuns in timpul alocat.")
        except requests.exceptions.ConnectionError:
            raise TranslationException("Nu s-a putut stabili conexiunea cu Google Translate API.")

        if response.status_code != 200:
            raise TranslationException(
                f"Google Translate API a returnat eroare {response.status_code}: {response.text}"
            )

        try:
            return response.json()["data"]["translations"][0]["translatedText"]
        except (KeyError, IndexError) as e:
            raise TranslationException(f"Raspuns neasteptat de la Google Translate API: {e}")
